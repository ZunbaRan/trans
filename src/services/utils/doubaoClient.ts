...
// 创建模块特定的日志记录器
const logger = createModuleLogger('doubao-client');
...
export class DoubaoClient {
  ...
  constructor(
    configTitle: string = 'doubao-pro-1.5',
    logDir: string = path.join(process.cwd(), 'public/logs/doubao')
  ) {
    this.logDir = logDir;
  ...
  private async loadConfig(configTitle: string): Promise<void> {
    try {
      ...
      this.model = 'ep-20250201221516-pv4zn';
    }
  }
  ...
  public async chat(
    ...
    while (retryCount <= maxRetries) {
      try {
        ...
        // 记录对话
        await LogConversationUtil.logConversation(this.logDir, messages, response.data, 'doubao');
        ...
        // 如果是最后一次重试，则记录错误并抛出
        if (retryCount >= maxRetries) {
          logger.error('Doubao API 请求错误', error);
          throw error;
        }
...
// 导出单例实例
export const doubaoClient = new DoubaoClient('doubao-pro-1.5'); 